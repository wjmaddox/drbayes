...
parser.add_argument('--fisher', action='store_true', help='whether to compute the eigenvalues of the fisher matrix')
parser.add_argument('--ag', action='store_true', help='to use second order autograd for fisher')
...
if args.fisher:
    ...
    kwargs = {'fvp_method': 'FVP_AG' if args.ag else 'FVP_FD'}
else:
    print('computing eigenvalues of the hessian')
    min_max_fn = min_max_hessian_eigs
    kwargs = {}

max_eval, min_eval, hvps, pos_evals, neg_evals = min_max_fn(model, loader, criterion, use_cuda=True, verbose=True, **kwargs)
