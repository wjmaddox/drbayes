...
from swag.posteriors import SWAGFD
...
fd = SWAGFD(model_cfg.base, *model_cfg.args, max_num_models = 20, num_classes = 100, **model_cfg.kwargs)
fd.cuda()
...
#mean = pca.mean_
u = sketch[args.PC1, :].copy()
u /= np.linalg.norm(u)
v = sketch[args.PC2, :].copy()
v /= np.linalg.norm(v)

M = np.vstack((u[None, :], v[None, :]))
trajectory_projection = np.dot(M, (sketch - mean[None, :]).T)
